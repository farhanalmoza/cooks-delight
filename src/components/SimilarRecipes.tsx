...
export default function SimilarRecipes({
  ...
  const { meals, loading, error } = useSimilarRecipes(category, excludeId, 6)
  ...
  if (!loading && !error && meals.length === 0) return null

  return (
    <div className="w-full rounded-4xl border border-dark/24 p-4 pt-10">
      <div className="mb-8 flex items-center justify-between">
        <h2 className="text-header-2 font-montserrat uppercase text-dark">Similar Recipes</h2>
      ...
      {loading && <p className="text-paragraph-2 text-dark/60">Memuat resep serupa...</p>}
